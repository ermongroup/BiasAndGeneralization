...
# data I/O
parser.add_argument('-d', '--data_set', type=str, default='clevr', help='clevr dataset')
...
parser.add_argument('--universal', type=str, default='red.green.brown.blue-cone.cylinder.sphere', help='Can be either cifar|imagenet')
parser.add_argument('--cors', type=str, default='color', help='Can be either cifar|imagenet')
parser.add_argument('--dataset', type=str, default='red_cone.blue_sphere', help='Can be either cifar|imagenet')
parser.add_argument('--data_str', type=str, default='../dataset/clevr-dataset-gen/dataset/', help='Can be either cifar|imagenet')


args = parser.parse_args()
print('input args:\n', json.dumps(vars(args), indent=4, separators=(',',':'))) # pretty print args

save_str = '(%s)(%s)(%s)'%(args.cors, args.dataset, args.universal)

def parse_time():
    return time.strftime("%Y.%m.%d-%H:%M:%S", time.localtime())

now_time = parse_time()
checkpoint_dir = '%s_checkpoint/%s/%s'%(args.data_set, save_str, now_time)
sample_dir = '%s_samples/%s/%s'%(args.data_set, save_str, now_time)
test_sample_dir = '%s_samples/%s/%s/test_sample'%(args.data_set, save_str, now_time)
...
if args.data_set == 'clevr':
    import data.clevr_data as clevr_data
    DataLoader = clevr_data.DataLoader
else:
    raise("unsupported dataset")
train_data = DataLoader(args.batch_size * args.nr_gpu, args.data_str, args.universal, args.dataset, args.cors, rng=rng, shuffle=True, return_labels=False)
...
with tf.Session() as sess:
    for epoch in range(args.max_epochs):
        ...
        if epoch == 0:
            ...
            if args.load_params:
                ckpt_file = checkpoint_dir + '/params_' + args.data_set + '.ckpt'
        ...
        if epoch % args.save_interval == 0:
            ...
            plotting.plt.savefig(os.path.join(sample_dir,'%s_sample%d.png' % (args.data_set, epoch)))
            plotting.plt.close('all')
            np.savez(os.path.join(sample_dir,'%s_sample%d.npz' % (args.data_set, epoch)), sample_x)

            # save params
            saver.save(sess, checkpoint_dir + '/params_' + args.data_set + '.ckpt')
