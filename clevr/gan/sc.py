...
flags.DEFINE_integer("input_height", 108, "The size of image to use (will be resized). [108]")
...
flags.DEFINE_string("data_str", "../dataset/clevr-dataset-gen/dataset/", "Path to dataset")
flags.DEFINE_string("universal", "red.green.brown.blue-cone.cylinder.sphere", "Universal set of the configurations")
flags.DEFINE_string("cors", "color", "color or shape?")
flags.DEFINE_string("dataset", "red_cone.blue_sphere", "Exception set")

flags.DEFINE_string("input_fname_pattern", "*.png", "Glob pattern of filename of input images [*]")
flags.DEFINE_string("checkpoint_dir", "checkpoint", "will be overwritten")
flags.DEFINE_string("sample_dir", "samples", "will be overwritten")
flags.DEFINE_string("test_sample_dir", "samples", "will be overwritten")
flags.DEFINE_boolean("train", False, "True for training, False for testing [False]")
flags.DEFINE_boolean("crop", False, "True for training, False for testing [False]")

flags.DEFINE_float("gp_coef", 10., "Learning rate of for adam [0.0002]")
flags.DEFINE_integer("wdf_iter", 50, "train discriminator for wdf_iter times in the first f_iter iterations")
flags.DEFINE_integer("f_iter", 50, "train discriminator for wdf_iter times in the first f_iter iterations")
...
def main(_):
    ...
    save_str = '(%s)(%s)(%s)'%(FLAGS.cors, FLAGS.dataset, FLAGS.universal)
    FLAGS.sample_dir = 'clevr_samples/%s/%s.%s/%s'%(save_str,FLAGS.model_name,FLAGS.base_name,parse_time())
    FLAGS.test_sample_dir = 'clevr_samples/%s/%s.%s/%s/test_sample'%(save_str,FLAGS.model_name,FLAGS.base_name,parse_time())
    FLAGS.checkpoint_dir = 'clevr_checkpoint/%s/%s.%s/%s'%(save_str,FLAGS.model_name,FLAGS.base_name,parse_time())
    print (FLAGS.sample_dir, FLAGS.checkpoint_dir)
    # need to modify the code a bit mainly on the checkpoint directory if generate the sample        
